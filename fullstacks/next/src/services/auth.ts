 ...
 * 流程：双方都拿到一对有效新 token，老 token 只被 revoke 一次（第二个 UPDATE 是
 * no-op），构成「令牌树」而非「令牌链」，破坏了 family_id 重放检测的预期。
 *
 * 生产化做法：用条件 UPDATE + RETURNING 做 single-consumer 抢占：
 *   UPDATE refresh_tokens SET revoked_at = now()
 *    WHERE id = $1 AND revoked_at IS NULL
 *   RETURNING id;
 * rowCount=1 的请求是赢家继续签新 token；rowCount=0 的请求按重放处理。
 *
 * 教学项目里没做，原因：单用户场景几乎触发不到；想保留 spec 文档里描述的
 * 线性 read-check-update 心智模型给读者。
 */
