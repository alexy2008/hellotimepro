"""根据标题生成胶囊正文与开启天数建议；标题为空时由 AI 一并生成标题。"""
...
def _prompt_template() -> str:
    ...
    return (
        ...
        "（openInDays，1~3650 整数）。只返回严格 JSON："
        '{"title":"...","content":"...","openInDays":30}。'
    )


def _build_prompt(title: str) -> str:
    # title 为空字符串时即"空标题模式"，由 LLM 自行构思标题。
    return _prompt_template().replace("{TITLE_OR_EMPTY}", title).replace("{TITLE}", title)
...
_FALLBACK_CAPSULES: list[tuple[str, str, int]] = [
    (
        "写给一个月后的自己",
...
def _local_fallback(auto_title: bool, title: str) -> tuple[str, str, int]:
    """LLM 不可用时的本地兜底。返回 (title, content, openInDays)。"""
    if auto_title:
        return random.choice(_FALLBACK_CAPSULES)
...
def suggest_capsule(req: CapsuleSuggestionRequest) -> CapsuleSuggestion:
    ...
    try:
        ...
        if auto_title:
            gen_title = _truncate_title(str(result.get("title", "")))
            if not gen_title:
                raise ValueError("empty title from LLM in auto-title mode")
            result_title = gen_title
        ...
        fb_title, content, days = _local_fallback(auto_title, title)
        if auto_title:
            result_title = fb_title
