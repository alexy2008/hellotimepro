...
CREATE        INDEX IF NOT EXISTS users_nickname_trgm_ix ON users USING gin (lower(nickname) gin_trgm_ops);
...
CREATE        INDEX IF NOT EXISTS capsules_title_trgm_ix   ON capsules USING gin (lower(title) gin_trgm_ops);
