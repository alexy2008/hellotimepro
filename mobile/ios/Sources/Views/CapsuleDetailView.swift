// 胶囊详情（移动版）：徽章/码/标题；已开→正文，未开→翻页时钟倒计时 + 到期自动开启轮询；
// 复制 8 位码 / 分享 / 收藏。返回由 NavigationStack 提供（无自定义返回键）。
// = desktop/swiftui CapsuleDetailView（NSPasteboard→UIPasteboard，布局改竖排适配手机）。
import SwiftUI
import UIKit
...
struct CapsuleDetailView: View {
            ...
            VStack(alignment: .leading, spacing: Space.s5) {
                if loading {
                    ...
                    VStack(spacing: Space.s4) {
                        ...
                        Button("去输入码") { store.selectTab(.open) }.buttonStyle(.ht(.ghost, .md))
                    }
                    .frame(maxWidth: .infinity).padding(.top, Space.s12)
                }
            }
    ...
    private func detail(_ c: CapsuleDetail) -> some View {
        ...
        VStack(alignment: .leading, spacing: Space.s5) {
            ...
            Text("创建于 \(DateUtil.fmtDateTime(c.createdAt))").font(.system(size: FontSize.xs)).foregroundStyle(Theme.textMuted)

            Text(c.title).font(.system(size: FontSize.xxxl, weight: .bold)).foregroundStyle(Theme.textPrimary)
    ...
    private func sealed(_ c: CapsuleDetail) -> some View {
        VStack(spacing: Space.s4) {
            ...
            (Text("开启于 ").foregroundStyle(Theme.textSecondary)
                ...
                .font(.system(size: FontSize.sm)).multilineTextAlignment(.center)
        }
    ...
    private var sep: some View {
        Text(":").font(.system(size: FontSize.xxl, weight: .bold)).foregroundStyle(Theme.textMuted).padding(.top, Space.s3)
    }
    ...
    private func footer(_ c: CapsuleDetail) -> some View {
        VStack(alignment: .leading, spacing: Space.s3) {
            ...
            HStack(spacing: Space.s3) {
                Button { copyCode(c.code) } label: { Label(codeCopied ? "已复制!" : "复制码", systemImage: codeCopied ? "checkmark" : "doc.on.doc") }
    ...
    private func copyLink(_ s: String) {
        UIPasteboard.general.string = "用胶囊码 \(s) 在 HelloTime Pro 打开（开启 → 输入码）"
        linkCopied = true
        Task { try? await Task.sleep(nanoseconds: 2_000_000_000); linkCopied = false }
    }
}

/// 翻页时钟单元：numericText 内容过渡。
    ...
    var body: some View {
        VStack(spacing: Space.s2) {
            Text(DateUtil.pad2(value))
                .font(.system(size: FontSize.xxxl, weight: .bold, design: .monospaced))
