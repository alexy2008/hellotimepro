// 关于页（移动版）：产品简介 + iOS 端原生技术栈 + 后端技术栈（from health）。
// = desktop/swiftui AboutView（技术栈改 SwiftUI iOS / Swift；Container→ScrollView）。
import SwiftUI

private let IOS_STACK: [(name: String, version: String, icon: String)] = [
    ("SwiftUI", "iOS 17+", "/static/icons/swiftui.svg"),
    ("Swift", "6", "/static/icons/swift.svg"),
]

private let IOS_SUMMARY =
    "本应用是 HelloTime Pro 的原生 iOS 端：用 SwiftUI 声明式重建整套界面，零 webview。" +
    "与 desktop/swiftui（macOS）共享逻辑层（模型 / APIClient / 会话 / 主题），仅视图层按移动 IA 重建——" +
    "底部 Tab Bar（广场/开启/创建/我的）替代桌面顶部导航，符合 iOS 审美。" +
    "URLSession async/await 直连同一套 /api/v1 契约（默认 :9080 反代），不持有任何后端逻辑；" +
    "状态用 Observation（@Observable），头像 SVG 由 SVGView 渲染（iOS 的 UIImage 不原生认 SVG）。"
...
struct AboutView: View {
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: Space.s8) {
                ...
                stackSection(title: "iOS 端技术栈", items: IOS_STACK.map { ($0.name, $0.version, $0.icon) }, summary: IOS_SUMMARY)
    ...
    private func stackSection(title: String, items: [(String, String, String)], summary: String) -> some View {
        VStack(alignment: .leading, spacing: Space.s4) {
            Text(title).font(.system(size: FontSize.xl, weight: .semibold)).foregroundStyle(Theme.textPrimary)
            VStack(alignment: .leading, spacing: Space.s4) {
                HStack(spacing: Space.s6) {
                    ForEach(items, id: \.0) { (name, version, icon) in
                        VStack(spacing: Space.s1) {
                            if let url = store.api.resolveAsset(icon), !icon.isEmpty {
                                RemoteSVGImage(url: url) { Color.clear }.frame(width: 40, height: 40)
                            } else {
                                RoundedRectangle(cornerRadius: Radius.md).fill(Theme.surface2).frame(width: 40, height: 40)
                            }
