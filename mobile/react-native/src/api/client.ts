...
// 移植自 frontends/react-ts/src/api/client.ts。
//
// 唯一改动：BASE 从 ""（Web 走 Vite 代理）改为 API_BASE（RN 直打后端 :9080）。
// 其余（refresh 单飞 + 401 重放 + Envelope 解包 + 端点封装）逐字保留。
//
// - access token 存内存（auth store）；refresh token 同样存 store + AsyncStorage
